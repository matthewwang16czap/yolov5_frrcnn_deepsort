...
def run(
    ...
    # only use 1 (person) and 37 (sports ball)
    names = [0] * 100
    names[1] = 'person'
    names[37] = 'sports ball'
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # get prediction
        pred = model(im)[0]
        pred = torch.cat((pred['boxes'], torch.unsqueeze(pred['scores'], 1), torch.unsqueeze(pred['labels'].float(), 1)), 1)
        # only use 1 (person) and 37 (sports ball), and only allow above 0.5 scores
        valid_pred = []
        for p in pred:
            if p[-1].int() in [1,37] and p[-2] > 0.5:
                valid_pred.append(p)
        pred = torch.vstack(valid_pred)
        pred = [pred]
